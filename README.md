...
- Comunication Protocol(Common Language),  Protokol jaringan adalah aturan atau format yg di gunakan untuk berkomunikasi dalam jaringan, jadi ini bisa di samakan dengan bahasa klo di manusia. Jadi penting untuk antar jaringan untuk menggunakan protokol yg sama atau setidaknya memahami protokol yg digunakan ppihak lain untuk beriteraksi dengan baik.
- IP Address (Way to Address),  karena setiap jaringan komputer memiliki IP Address ini sama kaya alamat fisik yg memungkinkan pesan di kirimkan ke alamat tepat.
- Network Connection (Media komunikasi), media komunikasi di jaringan melibatkan saluran atau media yang di gunakan untuk mengirimkan data antar perangkat, contoh : kabel Ethernet, jaringan nirkabel (wifi), dan lainnya
- Readiness and Active Engagement(Ketersediaan dan kemaunann untuk berkomunikasi), dalam jaringan inii merujuk ke ketersediaaan perangkat untuk menerima dan mengirim data, serta kemampuan perangkat untuk terhubung ke jaringan dan berpartisipasi dalam proses komunikasi.
- Securitty and Data Integrity(Openness and Caution), ini mengacu pada keamanan informasi yang dikirimkan melalui jaringan, memastikan bahwa data tidak dirusak atau diakses oleh pihak yang tidak sah.
...
- TCP/IP merupakan model jaringan yang dikembangkan sebelum model OSI.
- TCP/IP tidak hanya digunakan untuk internet, tetapi juga diimplementasikan di berbagai jaringan seperti, LAN (Local Area Network), MAN (Metropolitan Area Network)

Contoh TCP/IP digunakan :
-  Di internet, TCP/IP digunakan untuk mengirim data dari satu komputer ke komputer lain.
-  Di LAN, TCP/IP digunakan untuk menghubungkan komputer-komputer di dalam satu gedung atau kantor.
-  Di MAN, TCP/IP digunakan untuk menghubungkan LAN-LAN di dalam satu kota atau wilayah.
-  Di WAN, TCP/IP digunakan untuk menghubungkan LAN-LAN di berbagai negara atau benua.
...
- - -
- Ethernet adalah teknologi fisik, yg di gunakan untuk menghubungkan perangkat dalam sebuah jaringan
- Media fisik yg di gunakan Ethernet berupa kabel seperti, Kabel UTP, Kabel STP dan juga Kabel Fiber Optik
- Ethernet juga dapat menggunakan media fisik nirkabel, seperti Wi-Fi.
- Ethernet dapat dikaitkan dengan lapisan fisik (Physical Layer) dan lapisan data link (Data Link Layer) dalam model OSI
...
- - -
- Alamat MAC adalah identifikasi fisik unik yang diberikan kepada perangkat keras, seperti kartu jaringan atau adaptor jaringan, pada saat diproduksi.
- MAC address bekerja pada lapisan data link (Data Link Layer) dalam model OSI dan merupakan alamat fisik perangkat
- Alamat MAC tetap unik untuk setiap perangkat dan biasanya tidak berubah, sementara alamat IP bisa berubah tergantung pada konfigurasi jaringan.
- MAC address adalah untuk mengidentifikasi perangkat secara unik di dalam jaringan dan mengarahkan paket data ke perangkat yang tepat berdasarkan alamat MAC tujuan.
- MAC address (Media Access Control address) berhubungan erat dengan Ethernet karena MAC address adalah alamat fisik unik yang terkait dengan setiap kartu jaringan atau perangkat jaringan yang terhubung dalam jaringan Ethernet.

Jaadi kesimpulan nya Mac Address di berikan oleh produsen perangkat keras dan alamat tersebut tidak berubah, tidak seperti IP Address yang diberikan oleh DHCP server atau secara statis dikonfigurasi oleh administrator jaringan, dan dapat berubah. mac di gunakan un tuk identifikasi perangkat keras sedangkan ip untuk identifikasi jaringan
</b></details>
...
- - -
- Alamat MAC siaran (FF:FF:FF:FF:FF:FF) adalah alamat khusus yang digunakan dalam jaringan Ethernet untuk mengirimkan paket data kepada semua perangkat yang terhubung dalam jaringan lokal.
- Alamat MAC siaran (FF:FF:FF:FF:FF:FF) digunakan ketika sebuah perangkat ingin mengirimkan informasi atau pesan ke semua perangkat yang terhubung dalam jaringan lokal.
</b></details>
...
- Sebuah alamat IP (Internet Protocol address) adalah label numerik yang diberikan kepada setiap perangkat yang terhubung ke jaringan komputer yang menggunakan Internet Protocol (IP) untuk komunikasi data.
- Alamat IP memiliki dua versi utama, yaitu IPv4 yang menggunakan representasi 32-bit dalam format desimal titik dan IPv6 yang menggunakan representasi 128-bit dalam format heksadesimal

Fungsi Alamat IP :
- Alamat IP digunakan untuk mengidentifikasi secara unik setiap perangkat atau antarmuka jaringan di dalam jaringan tersebut.
- Alamat IP membantu dalam proses routing, di mana alamat ini digunakan untuk menentukan rute pengiriman paket data melalui jaringan, memastikan bahwa paket data sampai ke tujuan yang benar.
...
- - - 
- subnet mask adalah angka 32-bit yang digunakan untuk melakukan mask terhadap alamat IP. Subnet mask membagi alamat IP menjadi bagian-bagian network addresses (alamat jaringan) dan host addresses (alamat host).
- sSubnet mask dibuat dengan mengatur bit-bit jaringan menjadi "1" dan bit-bit host menjadi "0".
- dalam IPv4, subnet mask ditunjukkan dalam format desimal titik, seperti 255.255.255.0. Misalnya, dengan subnet mask tersebut, alamat IP 192.168.1.0 akan memiliki tiga oktet pertama (192.168.1) yang merujuk pada bagian jaringan, sementara oktet keempat akan menunjukkan alamat host di dalam jaringan tersebut.
- Subnet mask memungkinkan pengelompokan alamat IP yang lebih efisien dalam jaringan.
- Dalam setiap jaringan, dua alamat khusus di reserve: alamat pertama sebagai network address (network ID) dan alamat terakhir sebagai network broadcast address
- Alamat network digunakan untuk mengidentifikasi jaringan itu sendiri.
- alamat broadcast digunakan untuk mengirimkan pesan kepada semua perangkat dalam jaringan.
...
- Private IP addresses adalah kumpulan alamat IP yang ditetapkan secara khusus untuk digunakan dalam jaringan lokal (LAN - Local Area Network).
- Range alamat IP yang digunakan untuk jaringan lokal ini telah ditetapkan oleh Internet Assigned Numbers Authority (IANA) untuk penggunaan dalam jaringan privat dan tidak di-route langsung di internet publik.
- Selain tidak dapat diakses secara langsung dari internet publik, penggunaan private IP address membantu meningkatkan keamanan dan privasi perangkat yang terhubung dalam jaringan lokal.
-  penggunaan private IP address memungkinkan perangkat di dalam jaringan yang sama untuk terhubung ke layanan tersebut tanpa perlu melalui internet publik.
...
- Public IP adalah alamat IP yang dapat di akses dari internet public. 
- IP Public adalah alamat yang diberikan langsung ke perangkat atau router dan digunakan untuk mengidentifikasi dan berkomunikasi dengan perangkat di luar jaringan lokal.
- Public IP address diperlukan saat Anda ingin perangkat Anda dapat diakses atau berkomunikasi dengan perangkat di luar jaringan lokal.
- Ini adalah teknik di mana Anda mengkonfigurasi router Anda untuk mengarahkan lalu lintas yang masuk dari internet ke perangkat yang tepat dalam jaringan lokal.
- penggunaan private IP address sudah cukup untuk berkomunikasi karena mereka berada dalam jaringan yang sama.
...
- Bertanggung jawab untuk transmisi bit-bit data melalui media fisik seperti kabel, serat optik, atau gelombang radio.
- Fokus utamanya adalah pada spesifikasi perangkat keras dan kecepatan transmisi data, serta konfigurasi fisik, pengiriman, dan penerimaan bit-bit.

Data Link Layer (Layer 2) :
- Memastikan pengiriman yang handal dari satu node ke node lain dalam satu jaringan fisik.
- Membagi data ke dalam frame-frame dan menangani deteksi dan koreksi kesalahan pada tingkat fisik.
- Alamat MAC (Media Access Control) ada di sini, mengidentifikasi perangkat secara unik di jaringan lokal.

Network Layer (Layer 3) :
- Mengatur routing data dalam jaringan yang lebih besar.
- Bertanggung jawab untuk pengalamatan logis, penentuan rute terbaik untuk paket data, dan mengarahkan paket antara jaringan berbeda.
- Protokol seperti IP (Internet Protocol) beroperasi di sini.

Transport Layer (Layer 4) :
- Menangani transfer data antara node-node di jaringan.
- Menyediakan end-to-end communication, memecah data menjadi segmen-segmen yang lebih kecil, mengontrol aliran data, dan memastikan pengiriman data yang andal.
- Protokol seperti TCP (Transmission Control Protocol) dan UDP (User Datagram Protocol) beroperasi di lapisan ini.

Session Layer (Layer 5) :
- Membangun, mengelola, dan mengakhiri sesi komunikasi antara dua mesin.
- Mengontrol dialog antara aplikasi di dua perangkat yang berkomunikasi, memulai, menjaga, dan menghentikan koneksi.
    
Presentation Layer (Layer 6) :
- Bertanggung jawab untuk mengelola format data yang diterima oleh aplikasi.
- Menangani enkripsi, kompresi, serta konversi format data agar bisa dimengerti oleh aplikasi.

Application Layer (Layer 7) :
    - Layer teratas yang berinteraksi langsung dengan pengguna.
    - Menyediakan layanan dan protokol yang diperlukan untuk aplikasi, seperti HTTP, FTP, SMTP, dan lainnya.

</b></details>

<details>
<summary>For each of the following determines to which OSI layer it belongs: </summary><br><b>
Error correction - Data link Layer (Layer 2) :
    - Error correction biasanya terjadi pada tingkat frame, yang dikelola oleh data link layer. Layer ini menangani deteksi dan koreksi kesalahan yang terjadi pada level fisik dan mengatur pengiriman ulang jika terjadi kerusakan pada data.
    
Packets routing - Network Layer (Layer 3) :
    - Routing paket data antar jaringan merupakan tugas dari network layer. Layer ini mengatur pengalihan paket data melalui rute terbaik yang tersedia menuju tujuan akhirnya.

Cables and electrical signals - Physical Layer (Layer 1) :
    - Physical layer menangani aspek fisik dari transmisi data, termasuk pengiriman sinyal-sinyal elektrik atau optik melalui media transmisi seperti kabel atau serat optik.
    
MAC address - Data link Layer (Layer 2) :
    - Alamat MAC, yang digunakan untuk mengidentifikasi perangkat dalam jaringan lokal, beroperasi di data link layer.
    
IP address - Network Layer (Layer 3) :
    - Alamat IP adalah bagian dari network layer dan digunakan untuk mengidentifikasi perangkat di jaringan yang lebih besar serta menentukan rute pengiriman data antar jaringan.
    
Terminate connections - Session Layer (Layer 5) :
    - Penanganan pembentukan, pengelolaan, dan pengakhiran koneksi dilakukan oleh session layer. Layer ini bertanggung jawab untuk memastikan koneksi dan menyediakan kontrol atas sesi komunikasi antara dua perangkat.
    
3-way handshake - Transport Layer (Layer 4) :
    - 3-way handshake adalah proses yang terjadi di transport layer, khususnya dalam protokol TCP. Proses ini adalah langkah awal untuk membentuk koneksi antara pengirim dan penerima, memungkinkan kedua sisi untuk saling mengkonfirmasi kesiapan untuk memulai komunikasi.
</b></details>
...
Unicast : Unicast adalah skema pengiriman yg paling dasar, di mana data dikirimkan dari satu pengirim ke satu penerima. Ini adalah metode umun yg di gunakan
    
Broadcast : Broadcast adalah skema pengiriman di mana data dikirimkan dari satu pengirim ke semua perangkat dalam suatu jaringan. Biasanya digunakan untuk pengumuman, pesan sistem, atau ketika pengirim tidak mengetahui alamat penerima secara spesifik.
    
Multicast : Multicast adalah skema pengiriman di mana data dikirimkan dari satu pengirim ke sekelompok penerima. Metode ini lebih efisien daripada broadcast karena hanya mengirimkan data kepada penerima yang tertarik, sehingga mengurangi kepadatan jaringan.
    
Geocast : Geocast adalah skema pengiriman di mana data dikirimkan ke perangkat-perangkat dalam area geografis tertentu. Ini biasanya digunakan untuk layanan berbasis lokasi atau ketika pengirim mengetahui lokasi umum penerima.
    
Anycast : Anycast adalah skema pengiriman di mana data dikirimkan dari satu pengirim ke salah satu dari beberapa penerima yang ditentukan. Biasanya digunakan untuk penyeimbangan beban atau ketika pengirim tidak mempermasalahkan penerima spesifik yang menerima data.
...
- CSMA/CD (Carrier Sense Multiple Access / Collision Detection) adalah sebuah protokol yang digunakan dalam jaringan Ethernet khususnya pada jaringan yang menggunakan topologi bus atau kabel yang dibagi (shared medium).

- Tapi saat ini, CSMA/CD telah menjadi usang dan tidak lagi digunakan dalam jaringan Ethernet modern.

- Protokol CSMA/CD digunakan untuk mengatur akses ke media bersama (shared medium) seperti kabel jaringan yang digunakan oleh beberapa perangkat.

- Pada jaringan modern, terutama di jaringan Ethernet yang menggunakan topologi star dan switch, CSMA/CD tidak lagi diperlukan.

Langkah-langkah CSMA/CD :

- Sebelum mengirimkan frame data, perangkat melakukan pemeriksaan untuk mengetahui apakah ada perangkat lain yang sedang mengirimkan frame data.
- Jika tidak ada perangkat lain yang sedang mengirim, perangkat tersebut mulai mengirimkan frame datanya.
- Namun, jika dua perangkat mengirimkan data secara bersamaan, terjadi collision (tabrakan). CSMA/CD mendeteksi collision ini.
- Setelah terjadi collision, kedua perangkat akan mengirimkan sinyal "jam signal" ke semua perangkat di jaringan untuk memberitahukan bahwa collision terjadi.
- Setelah itu, kedua perangkat menunggu waktu acak sebelum mencoba mengirim kembali frame datanya. Ini dilakukan untuk menghindari tabrakan yang terjadi secara berulang.
- Setelah menunggu waktu acak, perangkat-perangkat tersebut kembali mencoba mengirimkan frame data dan siklus ini akan terus berulang.
...
Router : 
- Router adalah perangkat yang bertanggung jawab untuk menghubungkan dua atau lebih jaringan yang berbeda, baik itu jaringan LAN (Local Area Network) atau WAN (Wide Area Network).
- Operasinya terjadi pada layer jaringan (Layer 3) dalam model OSI. Router menggunakan tabel routing untuk menentukan rute terbaik bagi paket data agar sampai ke tujuan yang benar.
- Router menggunakan alamat IP untuk mengarahkan paket data antar jaringan. Ini berarti router memutuskan jalur terbaik untuk mengirim paket data ke jaringan tujuan berdasarkan alamat IP.
- Perangkat ini mampu memisahkan domain broadcast, sehingga mencegah kebocoran informasi ke jaringan lain dan meningkatkan keamanan.
- Router dapat mengelola lalu lintas jaringan, memberikan fitur firewall, dan melakukan fungsi jaringan lainnya, membuatnya menjadi tulang punggung utama dalam infrastruktur jaringan.

Switch :
- Switch adalah perangkat yang menghubungkan beberapa perangkat dalam suatu jaringan lokal (LAN).
- Operasinya terjadi pada layer data link (Layer 2) dalam model OSI. Switch menggunakan alamat MAC untuk mengarahkan paket data ke perangkat yang tepat di dalam jaringan lokal.
- Switch bekerja dengan cara mempelajari alamat MAC dari perangkat yang terhubung dan menyimpan informasi ini di dalam tabel MAC address. Hal ini memungkinkan switch untuk mengirim data hanya ke perangkat yang dituju, mengoptimalkan lalu lintas jaringan lokal dan menghindari tabrakan data.
- Karena switch menghubungkan perangkat-perangkat di dalam LAN, ia memungkinkan komunikasi yang cepat dan efisien antar perangkat dalam jaringan lokal.

Hub :
- Hub adalah perangkat yang sederhana dan operasinya terjadi pada layer fisik (Layer 1) dalam model OSI.
- Hub menghubungkan beberapa perangkat ke dalam satu jaringan dan bekerja dengan cara melakukan rebroadcast (broadcast ulang) paket data yang diterimanya ke semua perangkat yang terhubung, tanpa memperhatikan alamat tujuan.
- Hal ini dapat menyebabkan collision (tabrakan) data karena semua perangkat yang terhubung menerima data yang dikirimkan, terutama ketika beberapa perangkat mencoba untuk mengirim data secara bersamaan.
- Karena sifatnya yang melakukan broadcast, efisiensi jaringan dapat menurun dan keamanan data dapat terpengaruh. Oleh karena itu, hub jarang digunakan dalam jaringan modern.
...
- Collision Domain adalah area atau domain di dalam jaringan komputer di mana paket data dapat bertabrakan (collision) saat dikirim melalui media transmisi yang sama.
- Biasanya, collision terjadi ketika dua atau lebih perangkat mencoba untuk mengirimkan data melalui media komunikasi yang sama secara bersamaan.
- Dalam situasi ini, sinyal data dari satu perangkat dapat bertabrakan dengan sinyal data dari perangkat lain, menyebabkan gangguan atau kegagalan pengiriman yang menghasilkan kehilangan paket data.

Faktor-faktor yang memengaruhi ukuran collision domain : 
- Topologi Jaringan
		Dalam topologi jaringan seperti bus atau hub, di mana beberapa perangkat terhubung pada segmen jaringan yang sama, semua perangkat berbagi bandwidth yang sama dan berada dalam satu collision domain.
		
- Metode Akses Bersama
		Jaringan yang menggunakan metode akses bersama seperti Ethernet, di mana perangkat harus berbagi jalur komunikasi yang sama, memiliki collision domain yang lebih besar.

- Switching
		Penggunaan switch dalam jaringan modern dapat membatasi collision domain. Dalam jaringan dengan switch, setiap port biasanya memiliki collision domain sendiri karena switch mengisolasi lalu lintas antar port.
...
- Broadcast Domain adalah area atau domain di dalam jaringan komputer di mana paket data yang dikirimkan sebagai "broadcast" dapat diterima oleh semua perangkat yang terhubung dalam domain tersebut.
- Dalam konteks jaringan, broadcast biasanya digunakan untuk tujuan pengumuman atau penyebaran informasi kepada semua perangkat dalam suatu jaringan lokal.
- Pesan broadcast dikirim ke alamat broadcast khusus yang ditujukan untuk seluruh jaringan lokal, dan semua perangkat dalam broadcast domain akan menerima pesan tersebut.

hal yang mempengaruhi ukuran atau batasan broadcast domain : 
1. Topologi Jaringan : Dalam topologi jaringan seperti bus atau hub, di mana semua perangkat terhubung pada segmen jaringan yang sama, broadcast domain mencakup semua perangkat dalam segmen tersebut.
2. Subnetting : Subnetting dalam jaringan memungkinkan pembatasan area di mana pesan broadcast dapat mencapai. Pembagian jaringan menjadi subnet-subnet yang lebih kecil dapat membatasi area di mana pesan broadcast disebarkan.
3. Penggunaan VLAN (Virtual Local Area Network) : Dalam pengaturan jaringan yang menggunakan VLAN, domain broadcast dapat dibatasi ke dalam setiap VLAN, memisahkan lalu lintas broadcast di antara VLAN yang berbeda.
...
alasannya three collision domain :
- Karena setiap komputer terhubung ke switch melalui port yang berbeda maka terdapat tiga collision domains terpisah.
alasannya one broadcast domain : 
- karena semua komputer terhubung ke switch yang sama maka hanya ada satu broadcast domain.
...
- Routercs mengirim paket data.

- Router menggunakan tabel routing untuk menentukan rute terbaik bagi paket data tersebut agar sampai ke tujuan yang benar.

- Setelah rute terbaik ditentukan, router mengarahkan paket data ke jaringan tujuan yang sesuai dan melibatkan penggunaan protokol routing untuk mengirimkan paket data ke jaringan yang tepat.

- Router juga bertanggung jawab untuk mengatur lalu lintas data.

- Router membantu memisahkan domain broadcast, artinya router dapat membatasi distribusi pesan broadcast di antara jaringan yang berbeda berfungsi untuk mengurangi kebisingan di jaringan yang lebih besar.

- Router juga berfungsi sebagai firewall untuk melindungi jaringan dari ancaman luar.
